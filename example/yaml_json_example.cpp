...
users:
  ...
  - name: guest     # 访客用户
    roles: [user]
        ...
        // 保存到文件
        std::ofstream out_file("../example/config_with_comments.yaml");
        if (out_file.is_open()) {
            out_file << updated_yaml;
            out_file.close();
            std::cout << "已将带注释的配置保存到 example/config_with_comments.yaml" << std::endl;
        }
        ...
        // 保存到文件
        std::ofstream out_file2("../example/config_without_comments.yaml");
        if (out_file2.is_open()) {
            out_file2 << yaml_no_comments;
            out_file2.close();
            std::cout << "已将无注释的配置保存到 example/config_without_comments.yaml" << std::endl;
        }
